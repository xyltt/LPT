...
from evaluations import get_loss, evaluate, predict
...
def train(args, model, tokenizer, train_dataset):
    ...
    for _ in train_iterator:
        ...
        if (
            ...
            loss_scalar = (tr_loss - logging_loss) / local_step
            learning_rate_scalar = scheduler.get_lr()[0]
            logs["learning_rate"] = learning_rate_scalar
            logs["loss"] = loss_scalar
            logging_loss = tr_loss
...
def main():
    ...
    if args.method_type == 'generative':
        ...
        model = GPT2ForDiscriminativeClassification.from_pretrained(
            args.model_name_or_path,
            config=config,
        )
    else:
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_all_metric = train(args, model, tokenizer, train_dataset)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)

    if args.do_infer:
        best_model_path = os.path.join(args.output_dir, "best_model")
        best_model = os.path.join(best_model_path, 'pytorch_model.bin')
        if os.path.exists(best_model):
            if args.method_type == 'discriminative':
                model = GPT2ForDiscriminativeClassification.from_pretrained(best_model_path, config=config)
            else:
