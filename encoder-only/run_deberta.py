...
from models.modeling_deberta import DebertaPromptTuning
...
def main():
    ...
    if args.add_prompt_layer == 0 and args.per_gpu_train_batch_size == 32:
        exit(0)

    if args.generator_type == 'NPG' and args.num_prompt_tokens != 5:
        exit(0)
    ...
    model = DebertaPromptTuning.from_pretrained(
        args.model_name_or_path, 
        config=config, 
    )
    ...
    if args.generator_type is not None:
        model.deberta.encoder.add_prompt_generator()
    else:
    ...
    if args.do_infer:
        ...
        if results:
            ...
            test_log_dir = args.log_dir.replace('dev', 'test')
            if not os.path.exists(test_log_dir):
                os.makedirs(test_log_dir)
            if args.debug:
                fitlog.debug()
            if args.local_rank in [-1, 0]:
                fitlog.set_log_dir(test_log_dir, new_log=True)
                fitlog.add_hyper(args)
            fitlog.add_metric({"test": res_for_display}, step=args.max_steps)
            fitlog.add_best_metric({"test": {metric_key.replace("-", "_"): results[metric_key]}})
            fitlog.finish()  
